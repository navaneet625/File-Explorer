...
void printFiles() {
        ...
        cout << "Mode:Normal Mode ,Press : to switch From Normal mode to Command Mode";
        return;
}
...
void copyFile(string fileName, string dest) {
        ...
        //if file not there then create and copy cotent of src file
...
void delDir(string dir) {
        ...
        while ((entry = readdir(dp)) != NULL) {
                ...
                if (S_ISDIR(statbuf.st_mode)) {
                        ...
                        unlink(entry -> d_name);//delete the file name
                }
        }
        chdir("..");
        closedir(dp);
